...
* An X11 Server (like xMining) if you plan to run the GUI inside a Docker container.
...
1.  **Start the services:**
    ...
    ```
2.  **X11 Forwarding:** Since this is a GUI application, ensure your X11 server is running on your host machine. The `docker-compose.yml` is configured to look for `host.docker.internal:0`.
...
mvn javafx:run
```
*Make sure your database is running and accessible via the credentials in `DatabaseConnection.java`.*
